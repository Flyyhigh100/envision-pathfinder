...
const CANVAS_DIRECTION: CanvasTile[] = [
  { title: "Vision", blurb: "Why this matters.", target: { kind: "anchor", anchor: "vision" } },
  { title: "Strategy & Roadmap", blurb: "Now · Next · North Star.", target: { kind: "anchor", anchor: "roadmap" } },
  { title: "Training & Enablement", blurb: "Grow people from curious to confident.", target: { kind: "node", nodeId: "training" } },
];

const CANVAS_FLOW: CanvasTile[] = [
  { title: "Opportunity Intake", blurb: "One front door.", target: { kind: "node", nodeId: "intake" } },
  { title: "Assessment & Routing", blurb: "A short, honest check.", target: { kind: "node", nodeId: "assessment" } },
  { title: "Build · Reuse · Route", blurb: "Best placed by fit and capacity.", target: { kind: "node", nodeId: "route" }, accent: true },
];

const CANVAS_SUPPORT: { title: string; blurb: string }[] = [
  { title: "Product Line AI Resources", blurb: "First line for product line ideas." },
  { title: "Central AI Team", blurb: "Primary path for enterprise & leadership." },
  { title: "Shared Capacity", blurb: "Mutual support where needed." },
];

const CANVAS_ENABLERS: CanvasTile[] = [
  { title: "Governance & Boundaries", target: { kind: "node", nodeId: "governance" } },
  { title: "Value Tracking", target: { kind: "node", nodeId: "value" } },
  { title: "Support & Community", target: { kind: "node", nodeId: "portfolio" } },
  { title: "Citizen Development Studio", target: { kind: "node", nodeId: "route" } },
  { title: "Way of Working Tool", target: { kind: "anchor", anchor: "model-footnote" } },
];

function StrategyCanvasHero({ onSelect }: { onSelect: (t: StrategyTarget) => void }) {
  return (
    <section id="model" className="relative scroll-mt-20 border-b border-hairline bg-surface/30">
      <div className="mx-auto max-w-7xl px-6 pb-16 pt-14 md:pb-20 md:pt-20">
        ...
        >
          ...
          <h1 className="mt-4 text-3xl leading-[1.1] md:text-[42px]">
            One page. The whole way of working.
          </h1>
          <p className="mt-4 text-[15px] leading-relaxed text-muted-foreground">
            Direction guides the work. Work flows where it’s best placed. System enablers hold it all up.
          </p>
        </motion.div>
        ...
        >
          {/* Direction band */}
          <CanvasBand eyebrow="Direction" caption="Sets the why and the pace.">
            <div className="grid gap-3 md:grid-cols-3">
              {CANVAS_DIRECTION.map((t) => (
                <CanvasHeroTile key={t.title} tile={t} onSelect={onSelect} />
              ))}
            </div>
          </CanvasBand>

          <CanvasConnector label="guides" />

          {/* Flow spine */}
          <div className="rounded-2xl border border-teal/30 bg-teal-soft/30 p-5 md:p-7">
            <div className="mb-4 flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-teal" />
              <span className="text-[11px] font-medium uppercase tracking-[0.16em] text-teal">
                Flow of work
              </span>
              <span className="text-[11px] text-muted-foreground/70">· the spine</span>
            </div>
            <div className="grid gap-3 md:grid-cols-3">
              {CANVAS_FLOW.map((t, i) => (
                <div key={t.title} className="relative">
                  <CanvasHeroTile tile={t} onSelect={onSelect} emphasis />
                  {i < CANVAS_FLOW.length - 1 && (
                    <div className="pointer-events-none absolute right-[-12px] top-1/2 hidden -translate-y-1/2 text-teal/50 md:block">
            ...
            {/* Integrated support strip */}
            <div className="mt-6 border-t border-teal/20 pt-5">
              <div className="mb-3 text-[10.5px] uppercase tracking-[0.16em] text-muted-foreground/80">
                Support model · integrated
              </div>
              <div className="grid gap-2.5 md:grid-cols-3">
                {CANVAS_SUPPORT.map((s) => (
                  <a
                    key={s.title}
                    href="#roles"
                    className="group flex flex-col gap-0.5 rounded-lg border border-hairline bg-card/70 px-4 py-3 transition-colors hover:border-teal/40"
                  >
                    <span className="text-[12.5px] font-medium leading-snug">{s.title}</span>
                    <span className="text-[11.5px] leading-relaxed text-muted-foreground">{s.blurb}</span>
                  </a>
                ))}
              </div>
            </div>
          ...
          <CanvasConnector label="enables" />

          {/* Enablers band */}
          <CanvasBand eyebrow="System Enablers" caption="Persistent foundations across the whole model.">
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              {CANVAS_ENABLERS.map((t) => (
                <CanvasHeroTile key={t.title} tile={t} onSelect={onSelect} compact />
              ))}
            </div>
          </CanvasBand>
          ...
          <p
            id="model-footnote"
            className="mx-auto mt-12 max-w-3xl text-center text-[12.5px] leading-relaxed text-muted-foreground scroll-mt-24"
          >
            Product line and central AI citizen developers are one role family in two homes.
            Work goes to whoever is best placed to take it on — by origin, fit, skill, and capacity.
          </p>

          <div className="mt-8 flex justify-center">
            <a
              href="#overview"
              className="inline-flex items-center gap-1.5 rounded-full border border-hairline bg-card px-4 py-1.5 text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground transition-colors hover:border-teal/40 hover:text-teal"
            >
              Scroll for the detailed model
...
function CanvasBand({
  ...
  return (
    <div className="rounded-2xl border border-hairline bg-card/40 p-5 md:p-7">
...
function CanvasConnector({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center py-3">
      ...
      <span className="my-1 text-[10px] uppercase tracking-[0.16em] text-muted-foreground/60">
        {label}
      </span>
      <div className="h-5 w-px bg-hairline" />
    </div>
...
function CanvasHeroTile({
  ...
  tile: CanvasTile;
  onSelect: (t: StrategyTarget) => void;
  emphasis?: boolean;
  compact?: boolean;
}) {
  return (
    <button
      ...
      className={cn(
        "group relative flex h-full w-full flex-col gap-1.5 rounded-xl border bg-card text-left transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-ring",
        compact ? "px-4 py-3" : "p-5",
        emphasis
          ? "border-teal/40 hover:border-teal/70"
          : tile.accent
            ? "border-teal/40 hover:border-teal/60"
            : "border-hairline hover:border-teal/40",
      )}
    >
      <div className="flex items-start justify-between gap-2">
        <h3 className={cn("font-medium leading-snug", compact ? "text-[13px]" : "text-[14.5px]")}>
          {tile.title}
        </h3>
        <ArrowUpRight className="h-3.5 w-3.5 shrink-0 text-muted-foreground/40 transition-all group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-teal" />
      </div>
      {tile.blurb && !compact && (
        <p className="text-[12.5px] leading-relaxed text-muted-foreground">{tile.blurb}</p>
      )}
    </button>
  );
}
